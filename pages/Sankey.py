...
if selected_option in liste_hopitaux:
    fig_centre_sankey, _ = donnees_centre(df, selected_option) 
    ...
    st.download_button(label="Télécharger le graph au format html", data=fig_centre_sankey.to_html(full_html=False), file_name="Sankey_diagram.html")
...
if selected_option == 'TOUT':
    ...
    st.download_button(label="Télécharger le graph au format html", data=fig_tout_sankey.to_html(full_html=False), file_name="Sankey_diagram_tout.html")
